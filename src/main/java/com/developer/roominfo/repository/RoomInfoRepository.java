...
public interface RoomInfoRepository extends CrudRepository<RoomInfo, Long> {
	 ...
	 * @return
	 */
	@Query(value = "SELECT r.res_seq, rif.room_name, u.user_id, u.name AS uName, r.using_date, r.start_time, r.end_time "
	...
	@Query(value = "SELECT rif.room_seq, rif.room_name, rif.info, rif.room_img, rif.person, rif.price, s.open_time, s.end_time "
			+ "FROM studycafe s, room_info rif "
			+ "where s.cafe_seq = rif.cafe_seq "
			+ "AND s.host_id = :hostId "
			+ "AND rif.room_status = 0 "
	 ...
	 * 
	 */
	@Query(value =  "select rif.room_seq, rif.room_img, rif.info, rif.room_name, rif.person, rif.price, rif.status, rif.cafe_seq, h.host_id\r\n"
			+ "    from room_info rif, studycafe s, host_user h\r\n"
			+ "    where rif.cafe_seq = s.cafe_seq\r\n"
			+ "    And s.host_Id= h.host_Id\r\n"
			+ "    and rif.cafe_seq = :srSeq", nativeQuery = true)
